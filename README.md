...
- `:Qanda <prompt-template-name>` commands execute immediately, whereas the [Prompt template picker](#prompt-template-picker) will preview the prompt in the Prompt window and await a user command before proceeding.
- The `:Qanda <prompt-template-name>` command creates a new chat unless the user enters an [input placeholder](#template-placeholders) value that ends in a space character followed by a `+` character, in which case the new turn will be appended to the current chat.
...
- The `$input` placeholder initiates a new chat if the user input ends in a space followed by a `+` character (the suffix is ignored if the template is opened in the Prompt window).
...
- Opening a prompt template with the [prompt template picker](#prompt-template-picker) previews the expanded prompt in the [prompt window](#prompt-window); the Prompt window preview will be skipped if you execute a prompt template directly using the `:Qanda <prompt-template-name>` command.
