...
> [!NOTE]
> A user prompt is not the same as a model request; a model request includes the user prompt along with conversation context (previous requests and responses) and model options.
...
The _provider picker_ is opened with the `:Qanda /provider_selector` command and implements the following key-mapped commands:
...
Selects a model from a list of models belonging to the current provider. Opened with the `:Qanda /model_selector` command.
